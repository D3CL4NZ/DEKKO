...
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE global_config SET dm_channel = ?", value)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET log_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET error_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET admin_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET manver_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
            db.execute("UPDATE config SET exclude_logging_channels = ? WHERE guild = ?", channels, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET owner_role_id = ? WHERE guild = ?", role.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET admin_role_id = ? WHERE guild = ?", role.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET mod_role_id = ? WHERE guild = ?", role.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET bot_role_id = ? WHERE guild = ?", role.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET human_role_id = ? WHERE guild = ?", role.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET verified_role_id = ? WHERE guild = ?", role.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET mute_role_id = ? WHERE guild = ?", role.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET purgatory_role_id = ? WHERE guild = ?", role.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET sus_role_id = ? WHERE guild = ?", role.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET new_years_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET cn_new_years_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET vday_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET st_patricks_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET easter_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET cinco_de_mayo_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET j4_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET halloween_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET thanksgiving_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET christmas_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
...
Requested by: `DEKKO Command Processor`
Started: <t:{int(time.time())}:R>""")
                db.execute("UPDATE config SET wishlist_channel = ? WHERE guild = ?", channel.id, ctx.guild.id)
